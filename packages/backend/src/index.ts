...
import {
    ...
    updateBitBadgesApiSettings,
} from './services/bitbadgesApiService';
...
import { AgentConfig, ChatMessage, CommandResult, WalletConfig } from './types';
dotenv.config();
...
app.get('/api/wallet/status', async (req, res) => {
    try {
        ...
        res.json({
            ...
            bitbadgesAddress: bitbadgesAddress || null,
...
app.post('/api/wallet/broadcast', async (req, res) => {
    try {
        ...
        if (!txBody) {
            return res
                .status(400)
                .json({ error: 'Transaction body is required' });
        }
...
app.post('/api/settings', (req, res) => {
    try {
        const { apiKey, chainId, creatorAddress } = req.body;

        // Validate chainId
        const validChainIds = ['bitbadges-1', 'bitbadges-2'];
        if (chainId && !validChainIds.includes(chainId)) {
            return res.status(400).json({
                error: 'Invalid chain ID. Must be bitbadges-1 or bitbadges-2',
            });
        }

        // Validate creator address format if provided
        if (
            creatorAddress &&
            creatorAddress.trim() &&
            !creatorAddress.trim().startsWith('bb1')
        ) {
            return res.status(400).json({
                error: 'Invalid creator address format. BitBadges addresses must start with "bb1"',
            });
        }

        // Update settings
        updateUserSettings({ apiKey, chainId, creatorAddress });
        const updatedSettings = getUserSettings();

        // Reinitialize BitBadges API with new settings
        updateBitBadgesApiSettings(
            updatedSettings.apiKey,
            updatedSettings.chainId
        );
...
function formatDocsResponse(result: CommandResult): string {
    ...
    if (data.documentationAvailable !== undefined) {
        ...
        response += `📚 Documentation: ${
            data.documentationAvailable ? '✅ Available' : '❌ Unavailable'
        }\n`;
...
function formatProtoResponse(result: CommandResult): string {
    ...
    if (data.generatedMessage) {
        ...
        if (
            data.generatedMessage.breakdown &&
            data.generatedMessage.breakdown.creator
        ) {
            response += `**👤 Creator:** ${data.generatedMessage.breakdown.creator}\n\n`;
        }
        ...
        if (data.simulation) {
            ...
            response += `- Status: ${
                data.simulation.valid ? '✅ Valid' : '❌ Invalid'
            }\n`;
            response += `- Gas Used: ${data.simulation.gasUsed.toLocaleString()}\n`;
            response += `- Recommended Gas: ${data.simulation.recommendedGas.toLocaleString()} (includes ${Math.round(
                (data.simulation.gasBuffer / data.simulation.gasUsed) * 100
            )}% buffer)\n`;
        ...
        if (
            data.context &&
            Object.keys(data.context.addressMappings || {}).length > 0
        ) {
            response += `**🔄 Address Resolutions:**\n`;
            Object.entries(data.context.addressMappings).forEach(
                ([from, to]) => {
                    response += `- ${from} → ${to}\n`;
                }
            );
...
function formatWalletResponse(result: CommandResult): string {
    ...
    if (data.displayBalance && data.badge !== undefined) {
        ...
        response += `🏦 **Balance:** ${
            data.displayBalance || `${data.badge} $BADGE`
        }\n`;
        response += `📝 **Raw Balance:** ${
            data.rawBalance || `${data.ubadge} ubadge`
        }\n`;
...
async function initializeServer() {
    try {
        ...
        const bitbadgesOpenApiUrl =
            'https://raw.githubusercontent.com/BitBadges/bitbadgesjs/main/packages/bitbadgesjs-sdk/openapi/combined_processed.yaml';
    ...
    server.listen(PORT, () => {
        ...
        console.log(
            `Available plugins: ${pluginManager
        ...
        if (openApiService.isInitialized()) {
            console.log(
                `BitBadges OpenAPI loaded with ${
                    openApiService.getEndpoints().length
                } endpoints`
            );
        }
    });
